...
-- =====================================================
-- STABILIZED NETWORKING (Bypasses Blocks in Public Games)
-- =====================================================
...
-- Safe server bounce fallback
pcall(function()
    local serverBounce = ReplicatedStorage:FindFirstChild("_CharaServer")
    if not serverBounce and Workspace:FindFirstChild("_CharaServer") == nil then
...
-- Load your exact custom rbxmx assets
...
-- Safely anchor and position knife without breaking scaling
pcall(function()
...
local function RunCharaOnClient(fromPlayer, isSpecial)
    ...
    if fromPlayer == player then
        -- Safe local animator adjustment to prevent complete freeze-ups
        pcall(function()
            local originalAnimator = humanoid:FindFirstChildOfClass("Animator")
            if originalAnimator then originalAnimator.Waves = false end
        end)
        hrp.Anchored = true
    end
    ...
    if isSpecial then
        ...
        pcall(function()
            ...
            local imperio1 = eyeAttachClone:FindFirstChild("impacto1")
            ...
            if imperio1 and imperio1:IsA("ParticleEmitter") then
                imperio1.Enabled = true
                task.delay(0.15, function() imperio1.Enabled = false end)
            end
        ...
        task.delay(10.5, function()
            ...
            local heartInTarget = targetChar:FindFirstChildOfClass("Part")
            ...
            if heartInTarget then
                ...
                pcall(function() heartInTarget.Transparency = 0 end)
                task.delay(2, function()
                    if heartHighlight and heartHighlight.Parent then heartHighlight:Destroy() end
                    pcall(function() if heartInTarget then heartInTarget.Transparency = 1 end end)
                end)
            end
    ...
    task.delay(animLength, function()
        ...
        pcall(function() if torsoAttach.Parent then torsoAttach:Destroy() end end)
...
local function FireAll(action, ...)
    if remote then
        pcall(function() remote:FireServer(action, ...) end)
    else
        -- High-probability fallback loop: automatically executes actions locally if server blocks it
        if action == "CharaActivate" then
            RunCharaOnClient(player, ...)
        elseif action == "AtonementActivate" then
            RunAtonementOnClient(player, ...)
        end
    end
end
